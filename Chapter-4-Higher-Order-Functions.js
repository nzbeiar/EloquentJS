...
function dominantWritingDirection(str){
    let counted = countBy(str, char => {
        let script = characterScript(char.codePointAt(0));
        return script ? script.name : "none"}).filter(el => el.name !== "none");
    return counted.reduce((a,b) => a.count > b.count ? SCRIPTS.filter(el => el.name === a.name).map(el => el.direction)[0] :
        SCRIPTS.filter(el => el.name === b.name).map(el=>el.direction)[0]);
